...
class AddStockToWatchlist @Inject constructor(
    private val repository: WatchlistRepository
){
    operator fun invoke(stock: StocksEntity, watchlistEntity: WatchlistEntity) :Flow<Resource<Unit>> = flow{
        emit(Resource.Loading())
        val result = safeApiCall {  repository.addStockToWatchlist(stock, watchlistEntity)}
        emit(result)
    }
